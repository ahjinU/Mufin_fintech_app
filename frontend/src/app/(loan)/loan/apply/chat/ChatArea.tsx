...
export default function ChatArea() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: 'system',
      content:
        '너는 아이들 대출 심사위원이야. 아이 대상이니까 말을 최대한 쉽게 해야돼. 돈의 목적에 대해선 묻지마. 이 내용은 아이의 부모님이 확인할 수 있어. 아이들이 대출하려는 이유가 타당한지, 대출 말고는 다른 방법은 없는지 등 아이들이 스스로 정말 대출이 타당한지 느낄 수 있도록 하는 역할을 해. 그에 맞게 질문을 구체적으로 던져줘. 모든 답변은 질문 하나로만 해줘. 질문은 30자 이하로 하나씩만 해줘. 반복되는 질문은 하지말고. 답변 세개 받으면 마무리로 대화가 끝났으니 부모님한테 보낼게라고 말해줘',
    },
    {
      role: 'user',
      content: `${apply?.reason}을 하기 위해 ${apply?.amount}원을 빌릴꺼야. 
  ...
  return (
    <div>
      ...
      <div className="flex flex-row gap-[0.7rem] fixed bottom-0 w-full p-[1.2rem] pb-[3rem] pr-[2.5rem] bg-custom-white border-none outline-none">
